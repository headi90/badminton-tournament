...
export default function TournamentsPage() {
  ...
  async function createTournament() {
    ...
    if (!trimmed) return
    await db.addTournament(trimmed, format, date, location)
    setName('')
    setDate('')
    setLocation('')
    void load()
    ...
    <div className="max-w-2xl mx-auto py-8 px-4">
      ...
      <div className="border rounded-xl p-4 bg-white mb-6 space-y-3">
        ...
        <input
          ...
          onKeyDown={e => e.key === 'Enter' && void createTournament()}
      ...
      {tournaments.length === 0 ? (
        ...
        <div className="space-y-3">
          {tournaments.map(tournament => (
            <TournamentCard key={tournament.id} t={tournament} onRemove={deleteTournament} />
          ))}
        </div>
      )}
    </div>
